#LIBRARIES
    ...
    #The following list of libraries are required for the successful completion of
    #all parts of this script.
...
# DATA SETUP
    ...
    #The filepath for this dataset must be adjusted to that of the user.
  		#charity = read.csv("E:/STAT 897/Group Project/charity.csv")
  		charity = read.csv("./charity.csv")
  		charity.t = charity
		
		#Transformations of relevant predictors.
		...
		#Partitioning and subsetting of training sets.
		...
		#Partitioning and subsetting of validation sets.
		...
		#Partitioning and subsetting of test sets.
  		data.test = charity.t[charity$part=="test",]
  		n.test = dim(data.test)[1] # 2007
  		x.test = data.test[,2:21]

		#Standardization processes of training set.
		...
		#Standardization processes of validation set.
  		x.valid.std = t((t(x.valid)-x.train.mean)/x.train.sd) # standardize using training mean and sd
  		data.valid.std.c = data.frame(x.valid.std, donr=c.valid) # to classify donr
  		data.valid.std.y = data.frame(x.valid.std[c.valid==1,], damt=y.valid) # to predict damt when donr=1

		#Standardization processes of test set.
  		x.test.std = t((t(x.test)-x.train.mean)/x.train.sd) # standardize using training mean and sd
  		data.test.std = data.frame(x.test.std)

		#Data frames for training and validation sets converting donr to a factor.
...
#EXPLORATORY DATA ANALYSIS
		...
		# Predictors by damt (previously identified predictors are log-transformed first)
  		p$predstodamt <-  {
  		  data %>% 
  		    filter( part == 'train' ) %>% 
  		    filter( damt > 0 ) %>% 
  		...
  		p %>% 
  		  ...
  		  lapply( function(x){  
  		    ggsave( 
  		      ...
  		      plot = p[[x]]  ,
    #THE DIRECTORY FOR THIS PRODUCT MUST BE SET BY USER.
  		      #path = 'E:/STAT 897/Group Project/' 
  		      path = './output/' 
  		    ) 
  		  })
...
# PREDICTION MODELING
		...
		#RIDGE REGRESSION (RR)
			#Produce model matrices for ridge regression and lasso
		...
		#LASSO
			set.seed(53)
			lasso.cv.out = cv.glmnet(rr.train.x, rr.train.y, alpha=1) #uses matrix model from ridge regression
			...
			lasso.mse = mean((y.valid - lasso.pred)^2) # mean prediction error
		...
		#PRINCIPAL COMPONANTS
			...
			#validationplot(pcr.train.fit,val.type="MSEP", main="PCR Validation Plot")
		...
		#PARTIAL LEAST SQUARES
			...
			#validationplot(pls.train.fit,val.type="MSEP", main="PLS Validation Plot")
			par(mfrow=c(2,1))
			validationplot(pcr.train.fit,val.type="MSEP", main="PCR Validation Plot")
			validationplot(pls.train.fit,val.type="MSEP", main="PLS Validation Plot")
...
## Visuals of Models Results: Classification  ###################################################

	class.profit.cumulative <- {
	ls() %>% 
	  str_subset( '^profit' ) %>% 
	  lapply( function(x) {
	    y <- get(x)
	    tibble( 
	      Index = seq_along(y) ,
	      Model = case_when( 
	        x %like% 'ldaB' ~ 'LDA Base Model' ,
	        x %like% 'ldaF' ~ 'LDA Full Model' ,
	        x %like% 'ldaB' ~ 'Logit Model' ,
          T ~ 'Other') ,
	      Profit = y )
	  }) %>% 
  c( list(
	  {
  	# Tree Cumulative Profit
  	tree.pred %>% as.character %>% as.integer() %>% 
  	  sort( T ) %>% 
  	  { cumsum( 14.5 * . -2 ) } %>% 
  	tibble( 
  	  Index = seq_along(.) ,
	  ...
	  {
  	# SVM Cumulative Profit
  	svm.pred %>% as.character %>% as.integer() %>% 
  	  sort( T ) %>% 
  	  { cumsum( 14.5 * . -2 ) } %>% 
  	tibble( 
  	  Index = seq_along(.) ,
	  ...
	  {
	# Random Forest Cumulative Profit
	predict(model.rf,valid.dat,type="class") %>% 
    ...
    mutate( Label = paste0( '$', prettyNum( Profit , big.mark = ',' ) ) )
	}
	
	class.profit.cumulative %>% 
  ggplot( aes( x = Index , y = Profit , group = Model , color = Model ) ) + 
  geom_path()	+
	geom_point( 
	  data = class.profit.max ,
	  aes( x = Index , y = Profit ) , 
	  color = 'black' 
  ) +
	geom_text(
	  data = class.profit.max ,
	  ...
	  color = 'black' ,
	  #check_overlap = TRUE ,
	  position = position_jitter( height = 500 , width = 50 ) 
	  #vjust = "inward", hjust = "inward"
	)
	  data = class.profit.max ,
	  aes( x = Index , y = Profit ) , color = 'black' 
	class.profit.max %>% .$Profit %>% prettyNum( big.mark =',' )
